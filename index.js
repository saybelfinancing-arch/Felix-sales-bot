...
const voice = require('./voice-agent');   // голосовой AI-агент (ConversationRelay)
...
// FELIX (voice-script.js), переключает тайский/английский, сам кладёт трубку и
// после звонка присылает разбор: ЛПР, email, договорённости.
// Контракт возврата прежний: { sid, status } либо { message } при ошибке.
async function twilioCall(phone, product, meta = {}) {
  try {
    return await voice.placeCall(phone, 'FELIX', { ...meta, product: product || '' });
  } catch (e) {
    console.error('twilioCall failed:', e.message);
...
async function runCallCampaign(contacts, channel, threadTs, postFn) {
  ...
  for (const c of contacts) {
    ...
    try {
      const cr = await twilioCall(phone, null, { company: c.company, channel, threadTs, email: c.email, lpr: c.lpr_name });
      if (cr.sid) {
...
// ── Старые статичные <Say>-скрипты УДАЛЕНЫ ────────────────────────────────────
// Они зачитывали текст и клали трубку — диалога не было.
// Сценарии теперь живут в voice-script.js (правятся без изменения кода),
// а разговор ведёт voice-agent.js через Twilio ConversationRelay.
...
// ВАЖНО: app.listen() заменён на http.createServer — WebSocket голосового агента
// иначе не поднимется.
...
voice.attach({
  app,
  server,
  // Вызывается после КАЖДОГО звонка: Claude разобрал стенограмму
  onOutcome: async (result, meta) => {
    const ch = meta.channel || 'C0B53EXNKL1';   // #sales-russian-products-in-thailand
    const emoji = {
      success: '✅', callback: '🔁', refused: '🚫',
      gatekeeper: '🚧', voicemail: '📼', no_answer: '📵',
      busy: '📵', failed: '❌', wrong_number: '⚠️',
    }[result.outcome] || '📞';

    const lines = [
      `${emoji} *Звонок: ${meta.company || result.to}* — ${result.outcome}`,
      result.contact_name ? `• ЛПР: ${result.contact_name}${result.contact_role ? ' (' + result.contact_role + ')' : ''}` : null,
      result.email  ? `• Email: ${result.email}` : null,
      result.phone  ? `• Тел: ${result.phone}` : null,
      result.interest ? `• Интерес: ${result.interest}` : null,
      result.summary ? `• Итог: ${result.summary}` : null,
      result.next_step ? `• Дальше: ${result.next_step}` : null,
    ].filter(Boolean);

    await post(ch, lines.join('\n'), meta.threadTs).catch(e => console.error('post:', e.message));

    saveToObsidian('Felix', 'call_reports',
      `**${meta.company || result.to}** | ${result.outcome} | ${result.email || '—'} | ${result.summary || ''}`
    ).catch(() => {});

    // Полная стенограмма — отдельно, чтобы не засорять основной отчёт
    if (result.transcript) {
      saveToObsidian('Felix', 'call_transcripts',
        `### ${meta.company || result.to} — ${result.outcome}\n\n${result.transcript}`
      ).catch(() => {});
    }
  },
});
...
server.listen(PORT, () => console.log(
  `🤖 Felix running on port ${PORT} — Gmail + Sheets + Memory + Voice AI`
));